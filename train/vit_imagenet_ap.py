...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (gd, images, labels) in enumerate(train_loader):
            ...
            images = images.view(-1, 1024, 4*4*3) 
...
def vit_ap_train(args):
    ...
    # Create datasets
    train_set = ImageNetDataset(train_dir, fixed_length=1024, patch_size=4)
    val_set = ImageNetDataset(val_dir, fixed_length=1024, patch_size=4)
    ...
    model = VisionTransformer(img_size=128, patch_size=4, in_channels=3, num_classes=1000)
