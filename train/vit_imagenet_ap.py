...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (gd, images, labels) in enumerate(train_loader):
            ...
            images = images.view(-1, 514, 4*4*3) 
...
def vit_ap_train(args):
    ...
    # Create datasets
    train_set = ImageNetDataset(train_dir, fixed_length=514, patch_size=4, sths=[9])
    val_set = ImageNetDataset(val_dir, fixed_length=514, patch_size=4, sths=[9])
    ...
    model = VisionTransformer(img_size=128, patch_size=4, in_channels=3, num_classes=1000, seq_length=514)
